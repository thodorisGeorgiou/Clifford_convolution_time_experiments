...
template <typename T>
struct ReduceIndexFunctor<CPUDevice, T> {
...
class ReduceIndexOp : public OpKernel {
 public:
  explicit ReduceIndexOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES(context, input.dims() == indexes.dims() + 1,
    ...
    const size_t maxInd = input.dim_size(input.dims()-1);

	ReduceIndexFunctor<Device, T>()(context->eigen_device<Device>(), in, out, ind, maxInd, indLength);
 ...
 private:
  ...
  TF_DISALLOW_COPY_AND_ASSIGN(ReduceIndexOp);
...
#define REGISTER_CPU(T)                                          \
  REGISTER_KERNEL_BUILDER(                                       \
      Name("ReduceIndex").Device(DEVICE_CPU).TypeConstraint<float>("T"), \
      ReduceIndexOp<CPUDevice, T>);
REGISTER_CPU(float);
// REGISTER_CPU(int32);
...
#define REGISTER_GPU(T)                                          \
  REGISTER_KERNEL_BUILDER(                                       \
      Name("ReduceIndex").Device(DEVICE_GPU).TypeConstraint<float>("T"), \
      ReduceIndexOp<GPUDevice, T>);
REGISTER_GPU(float);
#endif  // GOOGLE_CUDA
